...
def SendJsonToServer(json_object):
    ...
    try:
        if not token:
            token = login()  # Obtain the token if not available or expired

        # Send the HTTP request to the server
        result = http_request(
            url=add_readings_uri,
            method='post',
            body=json_object,
            headers={"TOKEN": token}
        )
        ...
        # If the status code is not 200, retry with a new token
        if result.status_code != 200:
            print(f"Error: Received {result.status_code} from server.")
            print(f"Response: {result.content}")

            # Attempt to re-login and get a new token
            token = login()
            result = http_request(
                url=add_readings_uri,
                method='post',
            ...
            # If the second request also fails, return False
            if not result or result.status_code != 200:
                print(f"Error: Retry failed with status {result.status_code if result else 'No Response'}.")
                return False
...
def logic(packet):
    try:
        ...
        if token:
            ...
            sucess= ConvertSensorsToReadings_server(packet)
            if not sucess:
                SendPacketHoldingDataBase(packet)
                print("saving in holding server error")
